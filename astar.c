...
void generateGrid() {
	...
	for (x = 0; x < GRID_SIZE_X; x++) {
		for (y = 0; y < GRID_SIZE_Y; y++) {
			...
			grid[x][y]->id = id++;
			grid[x][y]->odd = 0;
			grid[x][y]->positionGrid.x = x;
			grid[x][y]->positionGrid.y = y;
		}
	}
	...
	for (x = 0; x < GRID_SIZE_X; x++) {
		for (y = 0; y < GRID_SIZE_Y; y++) {
			...
			grid[x][y]->G = 0;
			grid[x][y]->mark = 0;
			grid[x][y]->step_weight = 0;
			grid[x][y]->parent = NULL;

			// Check if there is a neighbour in each direction, if so, add it as neighbour
			if (/*grid[x + 1][y] && */x + 1 < GRID_SIZE_X) grid[x][y]->east = grid[x + 1][y];
			if (/*grid[x - 1][y] && */x > 0) grid[x][y]->west = grid[x - 1][y];
			if (/*grid[x][y - 1] && */y > 0) grid[x][y]->south = grid[x][y - 1];
			if (/*grid[x][y + 1] && */y + 1 < GRID_SIZE_Y) grid[x][y]->north = grid[x][y + 1];
		}
	}
}
